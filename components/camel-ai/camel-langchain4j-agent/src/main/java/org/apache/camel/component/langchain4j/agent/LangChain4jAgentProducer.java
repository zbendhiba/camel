...
public class LangChain4jAgentProducer extends DefaultProducer {
    ...
    private void addToolToResult(
        ...
        ToolExecutor toolExecutor = (toolExecutionRequest, memoryId) -> {
            ...
            return AiToolExecutor.execute(spec, arguments, exchange);
        };

        resultBuilder.add(toolSpec, toolExecutor);
        LOG.info("Added dynamic tool: '{}' - {}", toolSpec.name(), toolSpec.description());
    }

    /**
     * Converts a langchain4j {@link ToolExecutionRequest} arguments into a flat map. Keeps the langchain4j type at the
     * boundary so version upgrades surface as compile errors if the API changes.
     */
    ...
    private Map<String, AiToolSpec> discoverAiToolsByName(String tags) {
        ...
        LOG.info("Discovered {} AI tools for tags: {}", toolsByName.size(), tags);
        return toolsByName;
    }
