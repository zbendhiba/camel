...
IMPORTANT: *Deprecated since Camel 4.22.* The `spring-ai-tools` consumer is deprecated and will be removed in a future release. Use the xref:ai-tool-component.adoc[AI Tool Component] (`ai-tool:`) instead, which provides a framework-agnostic tool registry shared across all AI components (Spring AI, LangChain4j, OpenAI). See the xref:camel-4x-upgrade-guide-4_22.adoc[4.22 upgrade guide] for migration instructions.
...
IMPORTANT: This component is designed to work with the xref:spring-ai-chat-component.adoc[Spring AI Chat Component]. Define tools using `ai-tool` consumer endpoints, then use them via `spring-ai-chat` with the `tags` parameter. See the <<Integration with Spring AI Chat Component>> section below for details.
...
[source,java]
----
from("ai-tool:weatherTool?tags=weather&description=Get current weather for a location")
...
[source,java]
----
from("ai-tool:weatherTool?tags=weather" +
...
from("ai-tool:weatherTool?tags=weather,external-api" +
    "&description=Get current weather for a location")
    .to("https://weather-api.example.com");

from("ai-tool:timeTool?tags=time,utility" +
...
[source,java]
----
from("ai-tool:riskyOperation?tags=ops&description=Perform risky operation")
...
from("ai-tool:queryDb?tags=database" +
...
from("ai-tool:sendNotification?tags=notifications" +
...
from("ai-tool:myTool?tags=t&parameter.location=string")
    .process(exchange -> {
        String location = exchange.getIn().getHeader("location", String.class);
...
The Spring AI Tools component is designed to work seamlessly with the xref:spring-ai-chat-component.adoc[Spring AI Chat Component]. Define tools using `ai-tool` consumer endpoints, then reference them in `spring-ai-chat` endpoints using the `tags` parameter to enable function calling capabilities.

=== How the Integration Works

When you configure the `tags` parameter on a `spring-ai-chat` endpoint:

1. The chat component discovers all `ai-tool` consumer endpoints registered with matching tags
    ...
    public RouteBuilder chatWithToolsRoutes() {
        return new RouteBuilder() {
            ...
            public void configure() throws Exception {
                // Define tools using ai-tool consumer
                from("ai-tool:queryCustomers?tags=database" +
                ...
                from("ai-tool:sendEmail?tags=email" +
                ...
                from("ai-tool:createInvoice?tags=billing" +
